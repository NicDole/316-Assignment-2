...
\[
X_k = \sum_{m=0}^{N/2-1} x_{2m}\,\omega_{N/2}^{km}
    + \omega_N^{k}\sum_{m=0}^{N/2-1} x_{2m+1}\,\omega_{N/2}^{km}.
\]
If we define the two $N/2$-point DFTs
\[
E_k = \sum_{m=0}^{N/2-1} x_{2m}\,\omega_{N/2}^{km}, 
\qquad
O_k = \sum_{m=0}^{N/2-1} x_{2m+1}\,\omega_{N/2}^{km},
\]
then each output coefficient can be written as
\[
X_k = E_k + \omega_N^{k} O_k, 
\qquad
X_{k+N/2} = E_k - \omega_N^{k} O_k,\quad k = 0,\ldots,\frac{N}{2}-1.
\]
...
Because the 2D transform is separable, our implementations \verb|naive_dft_2d| and \verb|fft_2d| do not work on all $N^2$ pixels at once. Instead, they perform $N$ independent 1D transforms along the rows, then $N$ independent 1D transforms along the columns of the intermediate result. In big-$O$ terms, the total cost is therefore
\[
...
\subsection{FFT of the Original Image}
We began by loading the provided grayscale image and computing its 2D Fourier transform using our \verb|fft_2d| implementation. The image was padded to the nearest power-of-two dimensions to satisfy the requirements of the recursive Cooley–Tukey FFT. The magnitude spectrum was visualized in log scale, with the zero frequency shifted to the center of the plot.
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{experiment_1_1.png}
    \caption{Original Image vs. Log-Magnitude FFT}
\end{figure}
\\
...
\paragraph{Alignment with NumPy’s Output.}
When comparing our \verb|fft_2d| results to \verb|np.fft.fft2|, we needed to account for differences in frequency layout. NumPy’s output places the zero-frequency component at the \((0,0)\) corner, whereas our visualization uses \verb|fftshift| to center the low-frequency content for interpretability. To make the comparison valid, we applied the same shift to NumPy’s output before computing numerical differences. Once both spectra were aligned in this way, the magnitude and phase values matched up to floating-point precision, confirming that the indexing conventions rather than the transform itself were responsible for any apparent discrepancies.


\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{experiment_1_2.png}
    \caption{Comparing Custom FFT and NumPy FFT}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{experiment_1_3.png}
    \caption{Numerical Error Printout}
\end{figure}

\subsection{Image Denoising via Frequency Filtering}
...
\\
The figure below shows several denoised versions of the image corresponding to different cutoff radii.
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{experiment_2.png}
    \caption{Numerous Denoised Images}
\end{figure}
\\
At very small cutoff radii (e.g., \verb|radius_ratio = 0.05|), the reconstructed image becomes overly smooth and loses essential structural information. A medium-sized radius (e.g., 0.15–0.25) successfully removes high-frequency noise while preserving major edges and contours. Larger radii result in minimal denoising because most of the high frequencies are retained.\\
\\
...
\subsubsection*{Comparison of Denoising Procedures}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=1\textwidth]{experiment_3_1.png}
    \caption{Numerous Compressed Images}
\end{figure}
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{experiment_3_2.jpg}
    \caption{Image Compression Printout}
\end{figure}
\\
Below are observations for each compression level:
\begin{itemize}
    \item \textbf{0\% compression (original):} Full reconstruction with no artifacts.
    \item \textbf{50\% zeroed:} Image remains visually almost identical to the original.
    \item \textbf{90\% zeroed:} Slight smoothing appears, but overall structure remains extremely strong.
    \item \textbf{95\% zeroed:} More smoothing appears, but overall structure is still quite clear.
    \item \textbf{99\% zeroed:} Noticeable loss of detail. Major shapes are preserved but edges are blurred.
    \item \textbf{99.9\% zeroed:} Image becomes heavily degraded. The original structure is essentially unrecognizable. 
\end{itemize}
...
\subsection{Runtime Evaluation: Naïve 2D DFT vs. 2D FFT}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{experiment_4_1.png}
    \caption{Naïve 2D DFT vs. FFT Plot}
\end{figure}
\\
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{experiment_4_2.png}
    \caption{Naïve 2D DFT vs. FFT Specs Printout}
\end{figure}
\\
The runtime plot clearly illustrates the difference in asymptotic behavior:
\begin{itemize}
    \item The naïve 2D DFT grows roughly as $O\left(N^3\right)$, becoming prohibitively slow beyond $N=128$.
    \item The 2D FFt grows as $O\left(N^2\log N\right)$, allowing it to scale to much larger matrices efficiently
\end{itemize}
...
\\
The experimental results confirmed the expected theoretical behavior. The FFT reproduced NumPy’s \verb|np.fft.fft2| output up to floating-point precision, validating the correctness of our implementation. Frequency-domain denoising demonstrated how image noise is concentrated in high frequencies, while low-frequency masking produced clean reconstructions with minimal loss of detail. Compression experiments further showed that most natural image information is concentrated in a very small number of large-magnitude Fourier coefficients, allowing for substantial sparsification with acceptable reconstruction quality. Finally, the runtime analysis clearly illustrated the dramatic performance difference between the naïve 2D DFT and the 2D FFT: while the naïve method becomes impractical for moderately sized inputs, the FFT scales efficiently and remains fast even for large matrices.\\
\\
